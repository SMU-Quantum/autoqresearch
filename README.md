...
The merged artifact now reflects the paper structure: Maximum Independent Set
(MIS) and decomposed Capacitated Vehicle Routing Problem (CVRP) are both present
with code, instructions, logs, checkpoints, plots, and analysis tables.
...
Generated bytecode, `.DS_Store`, local environments, and real IBM credential
files are intentionally not tracked. Credential templates and run artifacts are
kept where they support reproducibility.
...
Use `hardware_runs/ibm_credentials.template.json` as the credential shape.
